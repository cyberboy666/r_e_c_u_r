...
class Feedback:
    ...
    @static
    def supports_midi_feedback(device_name):
    ...
    def feedback_shader_feedback(self, on):
        self.midi_input.midi_feedback_device.send(
    ...
    def refresh_midi_feedback(self):
        ...
        for n,shader in enumerate(self.midi_input.message_handler.shaders.selected_shader_list):
            #print ("%s: in refresh_midi_feedback, got shader: %s" % (n,shader))
            # show if layer is running or not
            if self.midi_input.message_handler.shaders.selected_status_list[n] == '▶':
